...
import { DEFAULT_PROPOSAL_CONTENT } from "@/lib/constants";
import type {
  BuildingSummary,
  EstimateDraft,
  FloorSummary,
...
function getProposalContent(content?: ProposalContent): ProposalContent {
  return {
...
function addLetter(
  ...
  y = drawParagraphs({
    doc,
    text: proposalContent.letterOfIntroduction,
    y,
  }) + 22;
  doc.setFont("helvetica", "normal");
...
function addExecutiveOverview(
  ...
  y = drawParagraphs({
    doc,
    text: proposalContent.executiveOverview,
    y,
  }) + 24;
...
function addAboutProvider(doc: jsPDF, proposalContent: ProposalContent) {
  let y = addSectionPage(doc, "Section 03", "About the Service Provider");

  y = drawParagraphs({
    doc,
    text: proposalContent.aboutServiceProvider,
    y,
  }) + 12;
  y += 12;
...
function addWalkthroughBreakdown(doc: jsPDF, estimate: EstimateDraft) {
  const model = getProposalModel(estimate);
  let y = addSectionPage(doc, "Section 09", "Walkthrough Breakdown");
  ...
  y = addSectionPage(doc, "Section 09 - Continued", "Documented Rooms");

  addSimpleTable({
    body: estimate.entries.length
      ? estimate.entries.map((entry) => {
          const buildingName =
            model.buildings.find(
          ...
          return [
            ...
            entry.cleaningFrequency
              ...
              : `Default: ${model.defaultFrequency}`,
          ];
        })
      : [["No rooms documented", "-", "-", 0, "-"]],
    columnStyles: {
      3: { halign: "right" },
    },
    doc: doc as AutoTableDocument,
    head: ["Room", "Building", "Floor", "Minutes", "Cleaning Frequency"],
    startY: y,
  });
}
...
function createProposalDocument(
  ...
  const proposalContent = getProposalContent(content);

  addCover(doc, estimate);
  addTableOfContents(doc);
  addLetter(doc, estimate, proposalContent);
  addExecutiveOverview(doc, estimate, proposalContent);
  addAboutProvider(doc, proposalContent);
  addWhyProgram(doc);
...
export function createProposalPdfBlob(
  estimate: EstimateDraft,
  content?: ProposalContent,
) {
  return createProposalDocument(estimate, content).output("blob");
}
...
export function exportProposalPdf(
  ...
  downloadBlob(createProposalPdfBlob(estimate, content), fileName);
